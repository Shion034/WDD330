...
export default class Auth {
  ...
  async login(callback) {
    ...
    try {
      ...
      const object = await makeRequest('login', 'POST', postData);
      // let's get the user details as well and store them locally in the class
      this.jwtToken = object.accessToken;
  ...
  async getCurrentUser(email) {
    try {
      ...
      const getUser = await makeRequest('users?email='+ email, 'GET', null, this.jwtToken);
      console.log(getUser);
      return getUser[0];
    } catch (error) {
      // if there were any errors display them
